  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/70 shadow-sm">
      <div className="container mx-auto h-16 flex items-center justify-between px-4">
        <Link to="/doctor/dashboard" className="flex items-center gap-2 font-heading text-xl font-bold">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `transition-colors hover:text-purple-600 ${isActive ? 'text-purple-600 font-medium' : 'text-gray-600'}`
              }
            >
              {item.label}
            </NavLink>
      ...
      {open && (
        <div className="md:hidden border-t bg-white/90 backdrop-blur">
          <nav className="container py-3 grid grid-cols-2 gap-3 px-4">
            {navItems.map((item) => (
              <NavLink key={item.to} to={item.to} className="text-sm text-gray-600 hover:text-purple-600" onClick={() => setOpen(false)}>
                {item.label}
              </NavLink>
            ))}
