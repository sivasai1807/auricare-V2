...
import {listPatientDoctorVideos, type Video} from "@/lib/supabase/videos";
  ...
  useEffect(() => {
    ...
    const loadVideos = async () => {
      ...
      try {
        ...
        if (patient) {
          const doctorVideos = await listPatientDoctorVideos(patient.id);
          setVideos(doctorVideos);
          
          // Save to localStorage as backup
          localStorage.setItem("patient_learning_videos", JSON.stringify(doctorVideos));
        }
      } catch (error) {
        console.error("Error fetching videos:", error);
        // Fallback to localStorage
        const savedVideosStr = localStorage.getItem("patient_learning_videos");
        if (savedVideosStr) {
          try {
    ...
    const channel = supabase
      .channel(`patient-videos-${user.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    >
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600 mt-2">
          Videos uploaded by your doctor
        </p>
      </div>
        ...
        <Card className="bg-white/70 backdrop-blur-sm border-0 shadow-xl">
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-500">
              Your doctor hasn't uploaded any videos yet.
            </p>
          </CardContent>
        </Card>
      ) : (
...
export default PatientLearningHub;
